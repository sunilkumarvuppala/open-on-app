-- ============================================================================
-- Migration 25: Fix open_letter function to handle self-sent letters
-- ============================================================================
-- WHAT: Updates open_letter function to allow senders to open their own letters
-- WHY: Self-sent letters (sender == recipient) were failing with "Invalid recipient configuration"
-- BEHAVIOR:
--   - Checks if caller is the sender (self-send case)
--   - Falls back to existing recipient checks if not sender
--   - Allows sender to open their own letter even if recipient config is incomplete
...
BEGIN
  ...
  -- Verify caller is recipient OR sender (ONLY for self-sent letters)
  -- First check: Is caller the sender? Only allow if this is a self-send
...
Allows both recipients and senders (for self-sent letters) to open the letter.
Verifies caller is either the recipient or the sender (for self-sends).';
