...
class Capsule(Base):
    ...
    reply: Mapped[Optional["LetterReply"]] = sa_relationship(
        "LetterReply",
        primaryjoin="Capsule.id == foreign(LetterReply.letter_id)",
        uselist=False,
        back_populates="letter"
    )
    identity_hints: Mapped[Optional["AnonymousIdentityHints"]] = sa_relationship(
        "AnonymousIdentityHints",
        primaryjoin="Capsule.id == foreign(AnonymousIdentityHints.letter_id)",
        uselist=False,
        back_populates="letter"
    )
    invites: Mapped[list["LetterInvite"]] = sa_relationship(
        "LetterInvite",
        primaryjoin="Capsule.id == foreign(LetterInvite.letter_id)",
        back_populates="letter"
    )
...
class LetterReply(Base):
    """
    Letter reply model - one-time acknowledgment replies from receivers to senders.
    
    This is an additive feature that does not modify existing letter functionality.
    Each letter can have at most one reply, enforced by UNIQUE constraint.
    
    Fields:
    - id: UUID primary key
    - letter_id: References the capsule (letter) this reply is for (UNIQUE)
    - reply_text: Reply text content (max 60 characters)
        - reply_emoji: Selected emoji from fixed set: ❤️ 🥹 😊 😎 😢 🤍 🙏
    - receiver_animation_seen_at: When receiver saw the animation (after sending)
    - sender_animation_seen_at: When sender saw the animation (when viewing reply)
    - created_at: Creation timestamp
    """
    ...
    # Reference to the letter (capsule)
    letter_id: Mapped[UUID] = mapped_column(
        PostgresUUID(as_uuid=True),
        ForeignKey("capsules.id", ondelete="CASCADE"),
        nullable=False,
        unique=True,  # One reply per letter
        index=True
    )
    
    # Reply content
    reply_text: Mapped[str] = mapped_column(String(60), nullable=False)
    reply_emoji: Mapped[str] = mapped_column(String(4), nullable=False)
    
    # Animation tracking (separate for receiver and sender)
    receiver_animation_seen_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True),
        nullable=True
    )
    sender_animation_seen_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True),
        nullable=True
    )
    ...
    # ===== Relationships =====
    letter: Mapped["Capsule"] = sa_relationship(
        "Capsule",
        back_populates="reply"
    )


class AnonymousIdentityHints(Base):
    """
    Anonymous identity hints model - progressive hints for anonymous letters.
    
    This is an additive feature that does not modify existing anonymous letter functionality.
    Allows senders to add optional hints (max 3) that reveal automatically over time
    before the sender's identity is revealed.
    
    Fields:
    - id: UUID primary key
    - letter_id: References the capsule (letter) these hints are for (UNIQUE)
    - hint_1: First hint text (max 60 characters, nullable)
    - hint_2: Second hint text (max 60 characters, nullable)
    - hint_3: Third hint text (max 60 characters, nullable)
    - created_at: Creation timestamp
    
    Constraints:
    - Only one hints record per letter (UNIQUE constraint on letter_id)
    - Hints can only be added to anonymous letters (enforced at application level)
    - Each hint max 60 characters (enforced at database level)
    """
    __tablename__ = "anonymous_identity_hints"
    ...
    letter_id: Mapped[UUID] = mapped_column(
        ...
        ForeignKey("capsules.id", ondelete="CASCADE"),
        nullable=False,
        unique=True,  # One hints record per letter
        index=True
    )
    
    # Hint text fields (all optional, max 60 characters each)
    hint_1: Mapped[Optional[str]] = mapped_column(String(60), nullable=True)
    hint_2: Mapped[Optional[str]] = mapped_column(String(60), nullable=True)
    hint_3: Mapped[Optional[str]] = mapped_column(String(60), nullable=True)
    ...
    letter: Mapped["Capsule"] = sa_relationship(
        "Capsule",
        back_populates="identity_hints"
    )


class LetterInvite(Base):
    """
    Letter invite model - allows sending letters to unregistered users via private invite links.
    
    This is an additive feature that does not modify existing letter functionality.
    Each letter can have at most one active invite (enforced by application logic).
    
    Fields:
    - id: UUID primary key
    - letter_id: References the capsule (letter) this invite is for
    - invite_token: Secure random token for public access (32+ chars, non-guessable)
    - created_at: Creation timestamp
    - claimed_at: Timestamp when invite was claimed (NULL if not claimed yet)
    - claimed_user_id: User ID who claimed the invite (NULL if not claimed yet)
    
    Constraints:
    - Only one active invite per letter (enforced by application logic)
    - Token must be unique and non-guessable (32+ characters)
    - Once claimed, cannot be claimed again
    """
    __tablename__ = "letter_invites"
    ...
    # User who claimed the invite (nullable if not claimed)
    # Note: No FK constraint because auth.users is in a different schema managed by Supabase Auth
    claimed_user_id: Mapped[Optional[UUID]] = mapped_column(
        PostgresUUID(as_uuid=True),
        nullable=True,
        index=True
    )
